  ...
  <div id="tags-view-container" class="tags-view-container">
    <ScrollPane ref="scrollPane" class="tags-view-wrapper" @scroll="handleScroll">
      <router-link
        ...
        @click.prevent.native="selectTag(tag, $event)"
...
export default {
  ...
  computed: {
    ...
    routes() {
      return this.$store.state.menus.sidebarRouters;
    },
    theme() {
  ...
  created() {
    ...
    this.$bus.on('refreshTagFun', this.refreshTagFun);
  },
  // 组件销毁
  beforeDestroy() {
    // 销毁全局事件
    this.$bus.off('refreshTagFun', this.refreshTagFun);
  },
  mounted() {
    this.initTags()
    this.addTags()
  ...
  methods: {
    ...
    isFirstView() {
      try {
        return this.selectedTag.fullPath === this.visitedViews[1].fullPath || this.selectedTag.fullPath === '/home'
      } catch (err) {
        return false
      }
    },
